    ...
    for attack in attack_list:
        ...
        for defense in defense_list:
            ...
            if defense == 'edm':
                ...
                defense_args=f"'task:imagenette;out_path:{out_dir}'"
            ...
            if policy == 'random':
                ...
                #command_transfer = f"python defenses/adversary/transfer_edm.py {policy} {vic_dir} {strat} {defense_args} --out_dir {out_dir} --batch_size {batch_size} -d {dev_id} --queryset {queryset} --budget {budget} --quantize {quantize} --quantize_args {quantize_args} --defense_aware {defense_aware} --recover_args {recover_params} --hardlabel {hardlabel} --train_transform {transform} --qpi {qpi} -w 5"
                ...
                #command_train = f"python defenses/adversary/jacobian_edm.py {policy} {vic_dir} {strat} {defense_args} --quantize {quantize} --quantize_args {quantize_args} --defense_aware {defense_aware} --recover_args {recover_params} --hardlabel {hardlabel} --model_adv {f_v} --pretrained {pretrained} --out_dir {out_dir} --testdataset {p_v} -d {dev_id} --queryset {p_v} --query_batch_size {batch_size} --budget 4800 -e {epochs} -b {training_batch_size} --lr {lr} --lr_step {lr_step} --lr_gamma {lr_gamma} --seedsize {seedsize} --epsilon {jb_epsilon} --T {T} -w 5"
