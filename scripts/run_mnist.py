...
if not (os.path.exists(os.path.join(proj_path,vic_dir,'checkpoint.pth.tar'))
    ...
    status = os.system(f"python defenses/victim/train_admis.py {p_v} {f_v} -o {vic_dir} -b 64 -d {dev_id} -e 50 -w 4 --lr 0.01 --lr_step 30 --lr_gamma 0.5  --oe_lamb {oe_lamb} -doe {oeset}")
    if status != 0:
    ...
    for attack in attack_list:
        ...
        if attack == 'ddae':
            ...
            if generate_shadow:
                ...
                status = os.system(f"python defenses/adversary/train_shadow.py {shadowset} {shadow_model} -o {shadow_path} -b 64 -d {dev_id} -e 5 -w 4 --lr 0.01 --lr_step 3 --lr_gamma 0.5  --num_shadows {num_shadows} ")
        ...
        for defense in defense_list:
            ...
            if defense == 'edm':
                ...
                defense_args=f"'task:mnist;out_path:{out_dir}'"
            ...
            if policy == 'random':
                ...
                command_train = f"python defenses/adversary/train.py {out_dir} {f_v} {p_v} --budgets {budget} -e {epochs} -b {training_batch_size} --lr {lr} --lr_step {lr_step} --lr_gamma {lr_gamma} -d {dev_id} -w 4  --vic_dir {vic_dir} --semitrainweight {semi_train_weight} --semidataset {semi_dataset}"
                ...
                #command_train = f"python defenses/adversary/jacobian_edm.py {policy} {vic_dir} {strat} {defense_args} --quantize {quantize} --quantize_args {quantize_args} --defense_aware {defense_aware} --recover_args {recover_params} --hardlabel {hardlabel} --model_adv {f_v}  --out_dir {out_dir} --testdataset {p_v} -d {dev_id} --queryset {queryset} --query_batch_size {batch_size} --budget 4800 -e {epochs} -b {training_batch_size} --lr {lr} --lr_step {lr_step} --lr_gamma {lr_gamma} --seedsize {seedsize} --epsilon {jb_epsilon} --T {T} -w 0"
                command_train = f"python defenses/adversary/jacobian.py {policy} {vic_dir} {strat} {defense_args} --quantize {quantize} --quantize_args {quantize_args} --defense_aware {defense_aware} --recover_args {recover_params} --hardlabel {hardlabel} --model_adv {f_v}  --out_dir {out_dir} --testdataset {p_v} -d {dev_id} --queryset {p_v} --query_batch_size {batch_size} --budget 4800 -e {epochs} -b {training_batch_size} --lr {lr} --lr_step {lr_step} --lr_gamma {lr_gamma} --seedsize {seedsize} --epsilon {jb_epsilon} --T {T}"
                status = os.system(command_train)
                if status != 0:
                    if not os.path.exists(os.path.join(out_dir,'params_transfer.json')):
