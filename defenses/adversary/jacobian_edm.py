...
class JacobianAdversary:
    ...
    def __init__(self, blackbox, budget, model_adv_name, model_adv_pretrained, modelfamily, seedset, testset, device,
        ...
        print('=> Obtaining predictions over {} seed samples using strategy {}'.format(len(self.seedset),
                                                                                       self.aug_strategy))
        Dx = torch.stack([self.seedset[i] for i in range(len(self.seedset))]).cpu()   # TODO
        print(Dx.shape)
        Dy = []
        self.Y_true = []
        self.ori_Y = []
...
def main():
    ...
    nworkers = 10
    test_loader = DataLoader(queryset, num_workers=nworkers, shuffle=False, batch_size=128)
    with torch.no_grad():
